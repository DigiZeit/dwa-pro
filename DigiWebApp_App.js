...
//                that.clickCounter += 1;
//                if (DigiWebApp.SettingsController.globalDebugMode) console.log('img2 touchstart clickCounter = ' + that.clickCounter);
...
//            	var that = DigiWebApp.ApplicationController;
//                that.clickCounter += 1;
//                if (DigiWebApp.SettingsController.globalDebugMode) console.log('img2 mousedown clickCounter = ' + that.clickCounter);
//                if(that.clickCounter > that.clickLimit) {
//                    that.showConfirmDialog();
//                }
                that.showConfirmDialog();
...
//    		});
    ...
    , enforceChefToolOnly: function() {
		...
		if (ChefToolOnly || hideOverride) {
			...
			try{$('[id=' + DigiWebApp.SettingsPage.content.daysToHoldBookingsOnDeviceGrid.id  + ']').each(function() { $(this).hide(); });}catch(e){}
		} else {
			...
			if (DigiWebApp.SettingsController.featureAvailable('411')) { 
				try{$('[id=' + DigiWebApp.SettingsPage.content.daysToHoldBookingsOnDeviceGrid.id  + ']').each(function() { $(this).show(); });}catch(e){}
			}
		}
...
//            	}
        ...
        if (DigiWebApp.SettingsController.featureAvailable('411')) {
        	$('#' + DigiWebApp.SettingsPage.content.daysToHoldBookingsOnDeviceGrid.id).show();
        } else {
        	$('#' + DigiWebApp.SettingsPage.content.daysToHoldBookingsOnDeviceGrid.id).hide();
        }
    ...
    , save: function() {
    	...
    	var daysToHoldBookingsOnDevice  = $('#' + M.ViewManager.getView('settingsPage', 'daysToHoldBookingsOnDeviceInput').id).val();
...
DigiWebApp.InfoPage = M.PageView.design({
    ...
    , content: M.ScrollView.design({
          ...
          childViews: 'logo versionLabel cordovaVersionLabel companyLabel streetLabel cityLabel emailLabel1 spacer1 emailLabel2 spacer2 spacer3 TMPVersionLabel spacer4 deviceinfo spacer5 connectioninfo spacer6 environmentinfo spacer7 localStorageUsage'
...
DigiWebApp.SettingsPage = M.PageView.design({
    ...
    , content: M.ScrollView.design({
        //  childViews: 'companyGrid passwordGrid connectionCodeGrid workerIdGrid autoTransferAfterBookTimeCheck autoTransferAfterClosingDayCheck autoSyncAfterBookTimeCheck remarkIsMandatory autoSaveGPSData GPSDataIsMandatory useTransitionsSetting grid'
          childViews: 'companyGrid passwordGrid connectionCodeGrid workerIdGrid autoTransferAfterBookTimeCheck autoTransferAfterClosingDayCheck autoSyncAfterBookTimeCheck remarkIsMandatory autoSaveGPSData useTransitionsSetting daysToHoldBookingsOnDeviceGrid grid'
        , daysToHoldBookingsOnDeviceGrid: M.GridView.design({
              childViews: 'daysToHoldBookingsOnDeviceLabel daysToHoldBookingsOnDeviceInput'
            , layout: M.TWO_COLUMNS
            , daysToHoldBookingsOnDeviceLabel: M.LabelView.design({
                value: M.I18N.l('daysToHoldBookingsOnDeviceLabel')
            })
            , daysToHoldBookingsOnDeviceInput: M.TextFieldView.design({
                contentBinding: {
                      target: DigiWebApp.SettingsController
